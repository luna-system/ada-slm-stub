#!/usr/bin/env python3
"""Fine-tune Qwen2.5-0.5B-INSTRUCT on ASL - with PROPER LABELS MASKING.

KEY FIX: Only compute loss on the OUTPUT tokens, not the input!
"""
...
OUTPUT_DIR = Path(__file__).parent / "ada-slm-v3"
...
class ASLDatasetMasked(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Create labels with masking for input tokens
        ...
        # Set all tokens before the actual response to -100 (ignored in loss)
...
def main():
    ...
    print("=" * 60)
    print("ADA-SLM v3 (MASKED LABELS - only train on output!)")
    print("=" * 60)
    ...
    print(f"  Sample: {num_masked} input tokens masked, {num_trained} output tokens trained")
    ...
    print("\n" + "=" * 60)
    print("STARTING TRAINING (only output tokens!)")
